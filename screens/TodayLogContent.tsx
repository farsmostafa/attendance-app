...
const TodayLogContent: React.FC<Props> = () => {
  ...
  const handleNextDay = () => {
    const currentDate = parseDateString(selectedDate);
    const now = parseDateString(getCurrentCairoDateString());
    currentDate.setDate(currentDate.getDate() + 1);
    if (currentDate <= now) setSelectedDate(formatDateString(currentDate));
  };

  const handleToday = () => setSelectedDate(today);

  const isToday = selectedDate === today;
  const canGoForward = parseDateString(selectedDate) < parseDateString(today);
...
const styles = StyleSheet.create({
  ...
  cellName: { flex: 2.5, marginHorizontal: 4 },
  cellTime: { flex: 1.2, marginHorizontal: 4 },
  cellStatus: { flex: 1, justifyContent: "center", alignItems: "center", marginHorizontal: 4 },
  cellHours: { flex: 0.8, marginHorizontal: 4 },
  statusBadge: { paddingVertical: 4, paddingHorizontal: 8, borderRadius: 4, fontSize: 11, fontWeight: "600", overflow: "hidden" },
